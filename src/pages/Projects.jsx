...
const ProjectCard = ({index, name, description,tags,image,source_code_link}) => {
  return(
    <motion.div variants={fadeIn("up","spring",index * 0.5,0.75)} className="mb-8">
      <Tilt
        options={{
          max: 45,
          scale:1,
        ...
        className="bg-gradient-to-r from-cyan-100 to-sky-300 p-5  rounded-2xl sm:w-[360px] w-full">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {tags.map((tag) => (
              <p key={tag.name} className={`text-[14px] bg-gradient-to-r from-fuchsia-500 to-cyan-500 bg-clip-text text-transparent font-bold`}>
